...
import { createSnake, moveSnake, growSnake, checkSelfCollision, isOppositeDirection } from '../../src/engine/snake';
...
describe('moveSnake', () => {
  ...
  it('removes tail segment when not growing', () => {
    ...
    const snake = {
      segments: [{ x: 3, y: 0 }, { x: 2, y: 0 }, { x: 1, y: 0 }],
      direction: 'right' as const,
      growPending: false,
      alive: true,
    };
    const moved = moveSnake(snake, 'right');
    expect(moved.segments).toEqual([{ x: 4, y: 0 }, { x: 3, y: 0 }, { x: 2, y: 0 }]);
  });
});
...
describe('checkSelfCollision', () => {
  ...
  it('returns true when head overlaps a body segment', () => {
    const snake = {
      segments: [{ x: 2, y: 0 }, { x: 3, y: 0 }, { x: 3, y: 1 }, { x: 2, y: 1 }, { x: 2, y: 0 }],
