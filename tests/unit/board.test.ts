...
describe("wrapPosition", () => {
  ...
  it("wraps positions beyond the right and bottom edges", () => {
    expect(wrapPosition({ x: 20, y: 21 }, board)).toEqual({ x: 0, y: 1 });
  });

  it("wraps positions beyond the left and top edges", () => {
    expect(wrapPosition({ x: -1, y: -2 }, board)).toEqual({ x: 19, y: 18 });
  });
});
